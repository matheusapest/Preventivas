...
@section('content')
    ...
    <div class="w-full space-y-4 px-3 py-3 sm:space-y-6 sm:px-6 lg:px-8">
        ...
        <x-layout.page-header title="Execução de Preventiva" description="Execução da preventiva de manutenção.">
            <x-slot:breadcrumb>
            ...
            <x-slot:actions>
                <div class="flex flex-col-reverse gap-2 sm:flex-row sm:items-center">
                    ...
                    {{-- BOTÃO DE VOLTAR --}}
                    <a href="{{ route('preventivas.execucao.index') }}"
                    ...
                    @if ($pendingUnits->isNotEmpty())
                        <button type="button" id="open-finalize-pending-modal-header" data-finalize-pending-open
                            class="hidden sm:inline-flex w-full items-center justify-center gap-2 rounded-lg border border-amber-300 bg-amber-500 px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-amber-600 active:scale-[0.98] sm:w-auto sm:text-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0 text-white" fill="none"
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="mt-2 flex flex-col gap-2.5 sm:flex-row sm:items-center">
                                ...
                                <select id="execution-unit" data-preventive-id="{{ $preventive->id }}"
                                        ...
                                        @php
                                            ...
                                            $pendingActivities = collect($unit['activities'] ?? [])
                                                ->filter(function ($activity) {
                                                    return is_array($activity) && !($activity['answered'] ?? false);
                                                })
                                                ->map(function ($activity) {
                                                    $activityModel = $activity['activity'] ?? null;

                                                    $cycleUnitActivity = $activity['cycle_unit_activity'] ?? null;
                                                    ...
                                                    return [
                                                        ...
                                                        'name' =>
                                                            ...
                                                            ($activityModel?->name ?? 'Atividade'),
                                                    ];
                                                })
                                                ->filter(fn(array $activity): bool => !empty($activity['id']))
                                                ->values()
                                                ->all();
                                        @endphp

                                        <option value="{{ $cycleUnit->id }}"
                                ...
                                <button type="button" id="start-unit-execution"
                        ...
                        <div class="block rounded-lg border border-amber-200 bg-amber-50 p-3 sm:hidden">
                            ...
                            <button type="button" id="open-finalize-pending-modal-mobile" data-finalize-pending-open
        ...
        <div class="rounded-xl border border-slate-200 bg-slate-50 p-3.5 sm:p-4">
            ...
            @php
                ...
                $percentage = $totalActivities > 0 ? round(($answeredActivities / $totalActivities) * 100) : 0;
            @endphp
            ...
            <div class="mt-2.5 sm:mt-3">
                ...
                <div class="h-2 overflow-hidden rounded-full bg-slate-200">
                    ...
                    <div class="h-full rounded-full bg-slate-700 transition-all" style="width: {{ $percentage }}%;">
                    </div>
                    ...
                    @foreach ($units as $unit)
                        @php
                            ...
                            $unitStatusLabel = match ($unitStatus) {
                                ...
                                default => $completed ? 'Concluída' : 'Em execução',
                            };

                            $unitStatusClasses = match ($unitStatus) {
                                'conforme' => 'bg-green-50 border-green-200 text-green-700',

                                'nao_conforme' => 'bg-red-50 border-red-200 text-red-700',
                        ...
                        <div
                            class="overflow-hidden rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm space-y-3 sm:space-y-4 sm:p-5">

                            {{-- ============================================================
                                 BLOCO 1 — INFORMAÇÕES DA UNIDADE OPERACIONAL
                            ============================================================= --}}
                            ...
                            <div class="rounded-xl border border-slate-200 bg-slate-50/50 p-3.5 sm:p-4">
                                ...
                                <div class="mt-3 border-t border-slate-200/60 pt-2.5 sm:mt-4 sm:pt-3">
                                    ...
                                    <div class="mt-1 h-1.5 overflow-hidden rounded-full bg-slate-200 sm:h-2">
                                        ...
                                        <div class="h-full rounded-full bg-slate-700 transition-all"
                                ...
                                <div class="space-y-3 sm:space-y-4">
                                    ...
                                    @foreach ($unit['activities'] as $activity)
                                        @php
                                            ...
                                            $activityFinalStatus = $activity['final_status'] ?? null;

                                            $failedComponents = $activity['failed_components'] ?? [];
                                            ...
                                            $activityTypeLabel = $activity['activity_type_label'] ?? '—';

                                            $responseLabel = match ($activityStatus) {
                                                'conforme' => 'Conforme',
                                                'nao_conforme' => 'Não conforme',
                                                default => 'Pendente',
                                            };

                                            $responseClasses = match ($activityStatus) {
                                                'conforme' => 'bg-green-50 border-green-200 text-green-700',
                                        ...
                                        <div
                                            class="rounded-xl border border-slate-200 bg-white p-3.5 space-y-3 sm:space-y-4 sm:p-4">
                                            ...
                                            <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3">
                                                ...
                                                <div class="min-w-0">
                                                    ...
                                                    <p
                                                        class="mt-0.5 break-words text-xs font-medium text-slate-700 sm:text-sm">
                                            ...
                                            <div>
                                                ...
                                                <div
                                                    class="inline-flex rounded-lg border px-2.5 py-1 text-xs font-semibold {{ $responseClasses }}">
                                                    {{ $responseLabel }}
                                                </div>
                                                ...
                                                <div class="border-t border-slate-100 pt-2.5 sm:pt-3">
                                                    ...
                                                    <div class="space-y-1.5 sm:space-y-3">
                                                        ...
                                                        @foreach ($failedComponents as $component)
                                                            ...
                                                            <div
                                                                class="flex min-w-0 items-center justify-between gap-2 rounded-lg border border-red-100 bg-red-50/80 px-2.5 py-1.5 sm:px-3 sm:py-2">

                                                                <span
                                                                    class="min-w-0 truncate text-xs font-medium text-red-800">
                                                                    {{ $componentName }}
                                                                </span>

                                                                <span
                                                                    class="shrink-0 rounded-md bg-red-100 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-red-700 sm:text-[10px]">
                                                                    Problema
                                                                </span>
                                            ...
                                            {{-- ============================================================
                                                 BLOCO 3 — SITUAÇÃO FINAL DO PDV
                                            ============================================================= --}}

                                            <div
                                                class="mt-3 rounded-xl border border-slate-200 bg-slate-50/80 p-3 space-y-2.5 sm:mt-4 sm:p-3.5 sm:space-y-3">
                                                ...
                                                <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3">
                                                    ...
                                                    <div class="min-w-0">
                                                        ...
                                                        <div
                                                            class="inline-flex items-center rounded-lg border px-2.5 py-1 text-xs font-semibold {{ $unitStatusClasses }}">
                                                            {{ $unitStatusLabel }}
                                                        </div>
                                                    ...
                                                    @if ($activityFinalStatus)
                                                        <div class="min-w-0">
                                                            ...
                                                            <div
                                                                class="inline-flex items-center rounded-lg border px-2.5 py-1 text-xs font-semibold {{ $activityFinalStatus->colorClass() }}">
                                                                {{ $activityFinalStatus->label() }}
                                                            </div>
                                                ...
                                                @if ($response?->observation)
                                                    <div class="pt-1">
                                                        ...
                                                        <div
                                                            class="break-words rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs leading-relaxed text-slate-700 sm:px-3 sm:py-2">
                                                ...
                                                @if ($response?->photo)
                                                    <div class="border-t border-slate-200 pt-2.5 sm:pt-3">
                                                        ...
                                                        <button type="button" data-photo-modal-open
                                                            ...
                                                            <img src="{{ route('preventivas.execucao.response-photo', [
                                                                'preventive' => $preventive->id,
                                                                'response' => $response->id,
                                                            ]) }}"
