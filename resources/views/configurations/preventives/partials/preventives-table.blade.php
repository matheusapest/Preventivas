<x-cards.card class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
    ...
    <div class="block divide-y divide-slate-100 lg:hidden">
        ...
        @forelse ($preventives as $preventive)
            ...
            @php
                ...
                $currentCycle = $preventive->current_cycle_model ?? null;

                $isRejected =
                    $currentCycle &&
                    $currentCycle->review_status === \App\Enums\CycleReviewStatusEnum::REJECTED;

                $isApproved =
                    $currentCycle &&
                    $currentCycle->review_status === \App\Enums\CycleReviewStatusEnum::APPROVED;

                /*
                 * Evita exibir "Reprovado" novamente caso a
                 * observação seja exatamente esse texto.
                 */
                $reviewObservation = trim((string) ($currentCycle?->review_observation ?? ''));

                $showReviewObservation =
                    $reviewObservation !== '' && mb_strtolower($reviewObservation) !== 'reprovado';
            @endphp
            ...
            <div class="space-y-3 p-3.5 sm:p-4">
                ...
                <div class="grid grid-cols-2 gap-x-3 gap-y-2.5 rounded-lg border border-slate-100 bg-slate-50/75 p-3 text-xs sm:grid-cols-3">
                ...
                <div class="grid grid-cols-2 gap-2 pt-1 sm:flex sm:flex-wrap sm:justify-end">
                    ...
                    {{-- VALIDAR --}}
                    @if ($preventive->status === \App\Enums\StatusPreventiveEnum::PENDING_APPROVAL)
    ...
    {{-- ============================================================
     VISÃO DESKTOP (Com contenção rígida de largura)
     ============================================================ --}}

<div class="hidden w-full max-w-full overflow-x-auto lg:block">

    <table class="w-full min-w-full table-auto divide-y divide-slate-200">

        <thead class="bg-slate-50">
            <tr>
                <th scope="col" class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                    Preventiva
                </th>
                ...
                <th scope="col" class="px-3 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">
                    Ações
                </th>
            </tr>
        </thead>
        ...
        <tbody class="divide-y divide-slate-200 bg-white">
            ...
            @forelse ($preventives as $preventive)
                ...
                @php
                    ...
                    $showReviewObservation =
                        trim((string) ($currentCycle?->review_observation ?? '')) !== '' &&
                        mb_strtolower(trim((string) ($currentCycle?->review_observation ?? ''))) !== 'reprovado';
                @endphp
                ...
                <tr class="transition hover:bg-slate-50">
                    ...
                    {{-- 2. PERFIL --}}
                    <td class="whitespace-nowrap px-3 py-3 text-xs text-slate-700">
                        {{ $preventive->preventiveProfile->name ?? '-' }}
                    </td>
                    ...
                    <td class="whitespace-nowrap px-3 py-3">
                        <span class="inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold {{ $statusClasses }}">
                            {{ $status->label() }}
                        </span>
                    </td>

                    {{-- 6. CICLO --}}
                    <td class="whitespace-nowrap px-3 py-3 text-xs font-semibold text-slate-800">
                        {{ $preventive->current_cycle ?? '—' }}
                    </td>
                            ...
                            <div class="flex flex-col">
                                <span
                                    @class([
                                        ...
                                        'bg-red-50 text-red-700' => $currentCycle->review_status === \App\Enums\CycleReviewStatusEnum::REJECTED,
                                        'bg-emerald-50 text-emerald-700' => $currentCycle->review_status === \App\Enums\CycleReviewStatusEnum::APPROVED,
                                        'bg-slate-50 text-slate-700' => !in_array($currentCycle->review_status, [\App\Enums\CycleReviewStatusEnum::REJECTED, \App\Enums\CycleReviewStatusEnum::APPROVED], true),
                                    ])
                                >
                                    {{ $currentCycle->review_status->label() }}
                                </span>
                                ...
                                @if ($isRejected && $showReviewObservation)
                                    <span class="mt-0.5 max-w-[120px] truncate text-[11px] text-slate-500" title="{{ $currentCycle->review_observation }}">
                    ...
                    <td class="whitespace-nowrap px-3 py-3 text-right">
                        <div class="flex items-center justify-end gap-1.5">
                            ...
                            <a
                                href="{{ route('preventivas.show', ['preventive' => $preventive->id]) }}"
                                class="inline-flex items-center rounded-md border border-slate-300 bg-white px-2.5 py-1 text-xs font-medium text-slate-700 shadow-sm transition hover:bg-slate-50"
                            >
                                Visualizar
                            </a>
                            ...
                            @if ($preventive->status === \App\Enums\StatusPreventiveEnum::PENDING_APPROVAL)
                                @can('validate', $preventive)
                                    <a
                                        href="{{ route('preventivas.validation', ['preventive' => $preventive->id]) }}"
                                        class="inline-flex items-center rounded-md bg-gray-900 px-2.5 py-1 text-xs font-medium text-white shadow-sm transition hover:bg-gray-700"
                                    >
                                        Validar
                                    </a>
                                @endcan
                            ...
                            @if ($preventive->can_continue)
                                <a
                                    href="{{ route('preventivas.continuation', ['preventive' => $preventive->id]) }}"
                                    class="inline-flex items-center rounded-md bg-amber-600 px-2.5 py-1 text-xs font-medium text-white shadow-sm transition hover:bg-amber-700"
                                >
                                    Novo Ciclo
                                </a>
                            @endif
